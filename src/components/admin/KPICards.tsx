  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
      ...
      <KPICard
        ...
        subtitle="Claims cancelled by customers"
        icon={
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <KPICard
        ...
        subtitle="Claims cancelled by customers"
        trend={data.cancellationRate < 0.1 ? 'up' : data.cancellationRate > 0.3 ? 'down' : 'neutral'}
        trendValue={data.cancellationRate < 0.1 ? 'Low' : data.cancellationRate > 0.3 ? 'High' : 'Moderate'}
        icon={
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
          </svg>
        }
        color={data.cancellationRate < 0.1 ? 'green' : data.cancellationRate > 0.3 ? 'red' : 'orange'}
      />
